...
**Access:** Fixed chat button (💬) bottom-right, always visible
...
| A: Conversational Q&A → saves result → marks task complete            |
| **3. Proactive Suggestions**                                          | AI detects user hesitation   | "💡 I noticed you're on licenses. For SaaS in CA, you'll likely need..."   |
| **4. Cross-Task Insights**                                            | User asks synthesis question | Q: "How much money do I need?"                                             |
| A: Pulls from Tasks 3.4, 3.6, 3.9 → comprehensive answer              |
| **5. Task Recommendations**                                           | User asks "what's next?"     | A: "Get your EIN from IRS (15 min, blocks 3 other tasks)" + [Start task →] |
...
| Use Case                                 | %   | Example                                                       |
| ---------------------------------------- | --- | ------------------------------------------------------------- |
| **"I don't understand"**                 | 40% | Form jargon unclear → AI explains in plain English            |
| **"What if my situation is different?"** | 25% | Edge cases outside structured flow → AI provides alternatives |
| **"Show me an example"**                 | 15% | User wants to see "good" → AI shows 3-5 relevant examples     |
| **"How do I actually do this?"**         | 10% | External action needed → AI gives step-by-step walkthrough    |
| **"Check my work"**                      | 10% | User wants validation → AI reviews against benchmarks         |
...
| `view_task`        | [View Entity Selection →]    | Navigates to task   |
| `apply_suggestion` | [Use this value proposition] | Fills form field    |
| `external_link`    | [Open IRS website ↗]         | Opens in new tab    |
...
- Provide specific legal/tax advice → "Consult a [lawyer/CPA]"
- Access external accounts ([IRS.gov](http://IRS.gov), banks)
- Make final decisions without user approval
- Guarantee accuracy for state laws → Cite sources, encourage verification
...
- Satisfaction (👍/👎 on responses)
- Action button clicks
- Tasks completed via chat vs UI

**Improve:**

- If many users ask same question → improve task instructions
- If AI frequently wrong → improve system prompt
- If users abandon after chat → simplify task
- If chat used more than UI → improve form UX
